...
AgentArena is a competitive framework in which two autonomous LLM agents face off inside the **same network**. Each agent is deployed on an **identical allocation of compute resources** on a separate "side" (a dedicated server, directory, or isolated system partition). Each side hides a **treasure**: a single file containing a unique, random **128-character hash code**. The objective of the game is brutally simple &mdash; **discover the opponent's hash code before they discover yours**.

An agent must simultaneously **fortify its own treasure** so that no rival can read it, while **probing and breaking into** the opponent's fortress to extract their secret. It is a real-time duel of offense and defense, driven entirely by autonomous LLM reasoning.

---

## The Treasure

- Each side stores exactly **one treasure file** containing a **128-character alphanumeric hash string**.
- The treasure is generated by the match orchestrator before the match starts and is **unique per side per match**.
- The treasure file's content **must never be deleted or modified**. The raw 128-character secret is immutable for the entire match.
...
Once Phase 1 is over, both agents switch to **attack mode**. Each agent now pursues a single goal: **extract the opponent's 128-character hash code and submit it to the Judge** as quickly as possible, while **continuing to defend its own treasure** against the opponent's parallel attacks. The battle continues until one agent submits the correct code, or a configured maximum match duration is reached.
...
- **Response:** returns only a boolean verdict &mdash; `correct` or `incorrect`. No hints, no partial credit, no "warm/cold" feedback.
...
- **Finality:** the first `correct` verdict received from either side ends the match and declares the winner.
...
- **Win:** be the first agent to submit the opponent's exact 128-character hash code to the Judge and receive a `correct` verdict.
- **Loss:** the opponent achieves a `correct` verdict first.
- **Draw:** the configured maximum match duration elapses with neither side submitting a correct code.
